...
#' @param L_cluster The predefined vectors of L, R and LRpair clusters.
#' @param R_cluster The predefined vectors of L, R and LRpair clusters.
#' @param LR_cluster The predefined vectors of L, R and LRpair clusters.
...
# Create circos plot 
# Inputs: 
# LRscorematrix: A matrix with columns "ligand", "receptor" and the condition names where the condition columns contain the LRscores of each ligand-receptor pairs in each condition.  
# LRscore_conditions: A vector of conditions to be used when calculating the maximum or mean LR expr_score for each LR pair acorss conditions
# L_cluster, R_cluster, LR_cluster: The predefined vectors of L, R and LRpair clusters.
# WhichLRscore: The LR expr_score to be used in the plot, which is 'LRscore_max' (for each LR pair, the maximum LR expr_score across LRscore_conditions) or 'LRscore_mean' (for each LR pair, the average LR expr_score across LRscore_conditions). 
# color_L: A vector of colors of the ligand clusters, with the name of each element the name of the corresponding cluster
# color_R: A vector of colors of the receptor clusters, with the name of each element the name of the corresponding cluster
# color_LR: A vector of colors of the LR clusters, with the name of each element the name of the corresponding cluster
...
# cplotthresh: A number. Only plot L-R edges in the circos plot with LRscore > cplotthresh
# cex: Font size


CircosPlot2 <- function(LRscorematrix, LRscore_conditions, L_cluster, R_cluster, LR_cluster, WhichLRscore,
  ...
  myLRscore = cbind.data.frame(LRscorematrix[,c('from','to')], LRscore_val, LRscore_val_max, LRscore_val_mean,
                             sprintf("%s_%s", LRscorematrix[,'from'], LRscorematrix[,'to']))
  colnames(myLRscore) = c(colnames(LRscorematrix), 'LRscore_max', 'LRscore_mean', 'L_R')
  
  ######################## Circos data
  LR_df = unique(myLRscore[,c('from', 'to', WhichLRscore, 'L_R')])
  ...
  Ligand = 'L'
  Receptor = 'R'
  LRP_clusters_pre = cbind(myLRscore[,'from'], myLRscore[,'to'], L_cluster[myLRscore[,'from']], R_cluster[myLRscore[,'to']], LR_cluster[myLRscore[,'L_R']])
  rownames(LRP_clusters_pre) = NULL
  colnames(LRP_clusters_pre) = c('L', 'R', 'L_cluster', 'R_cluster', 'LRcluster')
  LRP_clusters = cbind(LRP_clusters_pre, sprintf("%s_%s", LRP_clusters_pre[,Ligand], LRP_clusters_pre[,Receptor]))
  colnames(LRP_clusters) = c(colnames(LRP_clusters_pre), 'L_R')
  
  # L cluster
  L_clusterset = unique(LRP_clusters[,sprintf("%s_cluster", Ligand)])
  L_cluster_list = list()
  for (i in 1:length(L_clusterset)) {
    L_cluster_list[[i]] = unique(LRP_clusters[LRP_clusters[,sprintf("%s_cluster", Ligand)]==L_clusterset[i],Ligand])
  }
  ...
  # R cluster
  R_clusterset = unique(LRP_clusters[,sprintf("%s_cluster", Receptor)])
  R_cluster_list = list()
  for (i in 1:length(R_clusterset)) {
    R_cluster_list[[i]] = unique(LRP_clusters[LRP_clusters[,sprintf("%s_cluster", Receptor)]==R_clusterset[i],Receptor])
  }
  ...
  # LR cluster
  LR_clusterset = unique(LRP_clusters[,sprintf("%s%scluster", Ligand, Receptor)])
  LR_cluster_list = list()
  for (i in 1:length(LR_clusterset)) {
    LR_cluster_list[[i]] = unique(LRP_clusters[LRP_clusters[,sprintf("%s%scluster", Ligand, Receptor)]==LR_clusterset[i], 'L_R'])
  }
